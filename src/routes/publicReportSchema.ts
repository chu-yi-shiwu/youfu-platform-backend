...
export const reportSchema = z.object({
  ...
  phone: z.string().regex(/^1[3-9]\d{9}$/, '联系电话应为 11 位手机号').optional(), // 选填（用于回访）
  ...
  priority: z.enum(['urgent', 'normal', 'low']).optional(), // 报修端点选优先级覆盖（一般留空，由服务端推断；用户提供则优先采用，尊重用户明确意图）
  category_name: z.string().max(60).optional(), // 报修端点选分类名（兜底通道：AI 未识别时用户点选；服务端按名匹配 fault_category，匹配不到诚实保留名称不绑定 id）
  ...
  // ⑤ 扫码关联（DMR 补充录入通道）：报修人扫设备码(AST-)/目录码(CAT-)后透传，服务端按码精确关联资产/目录
  // DB 权威：查不到即诚实不关联，不臆造（保住诚实红线）。
  asset_code: z.string().max(64).optional(),    // 资产码（如 AST-3F-AIRCON-01，去掉前缀也接受）
  catalog_code: z.string().max(64).optional(),  // 目录码（如 CAT-electrician）
  // 合规硬护栏：提交即表示同意《隐私与录音照片留存告知》。缺省/ false 一律拒绝受理（绝不静默通过）
  consent: z.boolean().refine((v) => v === true, { message: '提交即表示同意《隐私与录音照片留存告知》' }),
}).refine(
  (b) =>
    (b.description?.trim().length ?? 0) >= 4 ||
    (b.attachments?.length ?? 0) > 0 ||
    (b.voice_media_ids?.length ?? 0) > 0 ||
    !!b.asset_code ||            // 纯扫码关联（无描述/照片）也应受理
    !!b.catalog_code,
  { message: '请至少提供一段描述、一张照片、一段录音或扫一个设备/目录码，我们才能受理' },
);
